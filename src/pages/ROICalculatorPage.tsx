    ...
    <div className="pt-32 pb-24 space-y-14 sm:space-y-20">
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl space-y-5">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-800 text-[11px] font-mono font-semibold"><Calculator className="w-3.5 h-3.5 text-zinc-600" /> Planning tool</div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-zinc-950 leading-[1.05]">Estimate what automation could be worth.</h1>
          <p className="text-base sm:text-lg text-zinc-600 leading-relaxed max-w-2xl">Use your current manual workload to model potential time savings. This is a planning estimate, not a promise of ROI.</p>
        </div>
      </motion.section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div className="lg:col-span-2 rounded-3xl bg-white border border-zinc-200 p-6 sm:p-7 shadow-xs space-y-6">
          <div><h2 className="text-lg font-bold text-zinc-950">Your current process</h2><p className="text-xs text-zinc-500 mt-1">Adjust the inputs to match one repetitive workflow.</p></div>
          {[
            ...
            <label key={label as string} className="block space-y-2">
              <div className="flex items-center justify-between gap-3"><span className="text-xs font-semibold text-zinc-700">{label as string}</span><span className="text-xs font-mono font-semibold text-emerald-600">{value as number}{(label as string).includes('cost') ? '' : (label as string).includes('automated') ? '%' : ''}</span></div>
        ...
        <div className="lg:col-span-3 rounded-3xl bg-zinc-950 text-white p-6 sm:p-8 shadow-xl relative overflow-hidden border border-zinc-800">
          ...
          <div className="relative space-y-7">
            ...
            <p className="text-xs text-zinc-500 border-t border-zinc-800 pt-4">The model assumes 4.33 weeks per month and applies the automation percentage to the current manual workload. Actual results depend on process complexity, implementation scope and software costs.</p>
          </div>
        </div>
      </section>

      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="rounded-3xl bg-zinc-950 border border-zinc-800 p-8 sm:p-10 text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-xl">
          ...
          <div className="flex flex-wrap gap-2 shrink-0"><button onClick={onOpenStartProject} className="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white text-zinc-950 text-sm font-semibold hover:bg-zinc-100 transition-colors">Discuss the workflow <ArrowRight className="w-4 h-4" /></button><button onClick={() => onNavigate('projects')} className="px-5 py-3 rounded-xl border border-zinc-800 text-zinc-300 text-sm font-semibold hover:bg-zinc-900 transition-colors">See projects</button></div>
        </div>
      </section>
    </div>
  );
...
const Metric = ({ icon, label, value }: { icon: React.ReactElement<{ className?: string }>; label: string; value: string }) => (
  <div className="rounded-2xl bg-white/[0.06] border border-white/10 p-4"><div className="text-emerald-400">{React.cloneElement(icon, { className: 'w-4 h-4' })}</div><div className="text-lg font-bold mt-3">{value}</div><div className="text-[10px] uppercase tracking-wider text-zinc-500 mt-1">{label}</div></div>
);
