...
export const Footer: React.FC<FooterProps> = ({ onNavigate, onOpenStartProject }) => (
  <footer className="flowlym-footer mt-12 text-white overflow-hidden">
    ...
    <div className="h-px bg-gradient-to-r from-transparent via-zinc-700 to-transparent opacity-80" />
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-8 relative">
      <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr_1.15fr] gap-7 items-start">
        ...
        <div>
          <h3 className="text-[10px] font-semibold uppercase tracking-[0.18em] text-zinc-500 mb-3">Explore</h3>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.045] p-4 backdrop-blur-sm shadow-[0_18px_50px_rgba(0,0,0,.18)]">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-[11px] text-zinc-500 mt-1">Tell us what you want to automate or build.</p>
            </div>
            <span className="footer-live-dot" />
          </div>
          <div className="flex flex-wrap gap-2 mt-3">
            <button onClick={onOpenStartProject} className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg bg-white text-zinc-950 text-[11px] font-semibold hover:bg-zinc-200 transition-all hover:-translate-y-0.5">
              Let's talk <ArrowRight className="w-3.5 h-3.5" />
            </button>
            <a href="https://www.instagram.com/flowlym.tech/" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-3.5 py-2 rounded-lg border border-white/10 text-zinc-300 text-[11px] font-semibold hover:border-fuchsia-400/60 hover:text-white transition-all">
              <Instagram className="w-3.5 h-3.5" /> Instagram
            </a>
          </div>
          <a href="mailto:info@flowlym.tech" className="flex items-center gap-2 mt-3 text-[11px] text-zinc-500 hover:text-white transition-colors">
            <Mail className="w-3.5 h-3.5" /> info@flowlym.tech
          </a>
        </div>
      </div>

      <div className="mt-6 pt-4 border-t border-white/8 flex flex-col sm:flex-row justify-between gap-2 text-[10px] text-zinc-600">
