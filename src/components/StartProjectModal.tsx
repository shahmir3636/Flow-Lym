...
export const StartProjectModal: React.FC<StartProjectModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-zinc-950/65 backdrop-blur-md modal-backdrop-in" onClick={onClose}>
      <div className="relative w-full max-w-xl bg-white rounded-[2rem] shadow-2xl border border-white/80 overflow-hidden text-zinc-900 modal-card-in form-modal-shell" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="relative flex items-start justify-between p-6 sm:p-7 border-b border-zinc-100 overflow-hidden">
          <div className="absolute -top-16 -right-10 w-40 h-40 rounded-full bg-violet-200/50 blur-3xl form-orb" />
          <div className="absolute -bottom-24 -left-16 w-48 h-48 rounded-full bg-cyan-100/60 blur-3xl form-orb form-orb-delay" />
          <div className="relative">
            <div className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-800"><Sparkles className="w-3.5 h-3.5 text-zinc-600" /> Let's build something useful</div>
            <h3 className="mt-2 text-xl sm:text-2xl font-bold tracking-tight">Start a Project</h3>
            <p className="text-xs sm:text-sm text-zinc-500 mt-1 max-w-md">Give us enough context to understand the opportunity. No sales script — just the project details.</p>
          </div>
          <button onClick={onClose} className="relative p-2 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded-xl transition-all"><X className="w-5 h-5" /></button>
        </div>
        ...
        {submitted ? (
          <div className="p-8 sm:p-10 text-center space-y-5 animate-in fade-in zoom-in-95 duration-300">
            ...
            <div><h4 className="text-2xl font-bold">Ready to send</h4><p className="text-zinc-600 text-sm max-w-sm mx-auto mt-2 leading-6">Your email app should open with the project details addressed to <span className="font-semibold text-zinc-900">info@flowlym.tech</span>.</p></div>
            <div className="p-4 bg-zinc-50 border border-zinc-200 rounded-2xl text-left text-xs space-y-2"><div><span className="text-zinc-500">Service:</span> <span className="font-semibold">{formData.serviceCategory}</span></div><div><span className="text-zinc-500">Timeline:</span> <span className="font-semibold">{formData.timeline}</span></div><div><span className="text-zinc-500">Company:</span> <span className="font-semibold">{formData.company || 'Not provided'}</span></div></div>
            <button onClick={handleReset} className="w-full py-3 bg-zinc-950 text-white text-sm font-semibold rounded-2xl hover:bg-zinc-800 transition-all">Done</button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="relative p-6 sm:p-7 space-y-4 text-sm">
            ...
            <div className="flex items-center gap-2 text-[11px] text-zinc-500"><Clock3 className="w-3.5 h-3.5 text-zinc-600" /> We review project inquiries personally and respond through email.</div>
            <button type="submit" className="group relative overflow-hidden w-full py-3.5 bg-zinc-950 text-white text-sm font-semibold rounded-2xl hover:bg-zinc-800 transition-all flex items-center justify-center gap-2 shadow-lg shadow-zinc-900/10 form-submit-button"><span>Send Inquiry</span><ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></button>
          </form>
        )}
      </div>
    </div>
...
const Field: React.FC<{ label: string; children: React.ReactNode }> = ({ label, children }) => <div className="space-y-1.5"><label className="text-xs font-semibold text-zinc-700">{label}</label>{children}</div>;
